...
const UserSidebar: React.FC<UserSidebarProps> = ({ profiePic, userName }) => {
  // Type for the navLinkStyle function, using NavLinkProps
  const navLinkStyle = ({
    isActive,
  }: {
    isActive: boolean;
  ...
  // Type the handleSignOut function
  const handleSignOut = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
    try {
      const res = await axios.get("http://localhost:4451/auth/logout");
